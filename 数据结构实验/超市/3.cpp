...
const int maxsize = 100; //链表数组的最多个数
int Goodnum = 0;		 //输入的商品数
int Adminnum = 0;		 //输入的管理员人数
int buyernum = 0;		 //输入的普通用户人数

//定义管理员结构体
typedef struct Admin *apointer;
...
typedef apointer alklist; //定义单链表类型
...
{
	apointer p;
	ofstream ofile("./管理员信息.txt", ios::out);
	ofile << "管理员人数："
		  << " " << Adminnum << endl;
	//文本第一行显示共有多少管理员
...
int Admin_ender(alklist &head, string ID, string mima)
{
	...
	while (p != NULL)
	{
		if (p->Admin_ID == ID && p->Admin_mima == mima)
			return 1;
		else
			p = p->next;
	}
...
alklist Admin_get(alklist &head)
{
	...
	head->next = NULL;
	p = head;
	ifstream ifile("./管理员信息.txt", ios::in);
	if (!ifile)
	{
		cerr << "管理员信息查询出错" << endl;
		return 0;
...
//普通用户结构
typedef struct custom *cpointer;
...
{
	cpointer p;
	ofstream ofile("./普通用户信息.txt", ios::out);
	ofile << "普通用户人数："
		  << " " << buyernum << endl;
...
clklist custom_get(clklist &buyer)
{
	...
	buyer->next = NULL;
	p = buyer;
	ifstream ifile("./普通用户信息.txt", ios::in);
	if (!ifile)
	{
		cerr << "用户信息查询出错" << endl;
		return 0;
...
//商品信息验证
int Good_ender1(sqlist &Goods, string ID)
{
	//对比相同返回1
	for (int i = 0; i < Goodnum; i++)
	{
		if (Goods.Good[i].Good_ID == ID)
			return 1;
	}
	return 0;
}
...
//名称验证
int Good_ender2(sqlist &Goods, string name)
{
	//对比相同返回1
	for (int i = 0; i < Goodnum; i++)
	{
		if (Goods.Good[i].Good_name == name)
			return 1;
	}
	return 0;
}
...
{
	int i = 0;
	while (i < Goodnum && Goods.Good[i].Good_ID != ID)
		i++;
...
{
	int i = 0;
	while (i < Goodnum && Goods.Good[i].Good_name != name)
		i++;
...
{
	ifstream ifile("./商品信息.txt", ios::in);
	if (!ifile)
	{
		cout << "文件打开失败！" << endl;
		return;
	}
	string IDtitle, nametitle, piecetitle, lasttitle; //用于显示（货品编码货品名字单价存货量）标题
...
{
	ofstream ofile("./商品信息.txt", ios::out);
	ofile << "商品种类："
		  << "\t\t" << Goodnum << endl;
	ofile << "商品编号"
		  << "\t\t"
...
void Good_add(sqlist &Goods)
{
	string ID, name;
	double piece;
	int last, sign, flat = 1;
	fstream ifile("./商品信息.txt", ios::in);
	...
	for (; flat != 0;)
	{
		...
		case 2:
		{
			for (int i = 0; i < Goodnum; i++)
			{
				for (int j = Goodnum; j >= i + 1; j--)
				{
					if (Go.Good[j].piece < Go.Good[j - 1].piece)
					{
						//比较单价大小，小的在前面，大的在后面
						flag = 1;
						p = Go.Good[j];
		...
		case 3:
		{
			for (int i = 0; i < Goodnum; i++)
			{
				for (int j = Goodnum; j >= i + 1; j--)
				{
					if (Go.Good[j].last < Go.Good[j - 1].last)
					{
						//比较编号大小，小的在前面，大的在后面
						flag = 1;
						p = Go.Good[j];
...
//购买商品
void Good_buy(sqlist &Goods, clklist &head, string cID)
{
	string ID, name;
		...
		case 1:
		{
			...
			if (Good_ender1(Goods, ID) == 1)
			{
				...
				if (shu <= Goods.Good[sign].last)
				{
					...
					cout << "购买成功" << endl;
					p->money = p->money - Goods.Good[sign].piece * shu;
					custom_save(head);
...
void show()
{
	...
	while (a != 0)
	{
		...
		case 1:
			...
			break; //超级管理员功能界面
		case 2:
			Admin_show(Ad, buyer, Goods);
			break;
		case 3:
			custom_show(buyer, Goods);
			break;
		case 0:
		{
			int ttt = 30;
